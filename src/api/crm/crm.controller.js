...
const MONEY = ['estimatedValuePaise'];
...
const list = asyncHandler(async (req, res) => {
  ...
  sendList(res, req, maskRateFields(data, req, MONEY), 'Leads retrieved');
});

const get = asyncHandler(async (req, res) => {
  sendSuccess(res, maskRateFields(await CrmService.getView(req.params.id), req, MONEY), 'Lead retrieved');
});

const create = asyncHandler(async (req, res) => {
  sendSuccess(res, maskRateFields(await CrmService.create(req.body), req, MONEY), 'Lead created', 201);
});

const update = asyncHandler(async (req, res) => {
  sendSuccess(res, maskRateFields(await CrmService.update(req.params.id, req.body), req, MONEY), 'Lead updated');
});

const setStatus = asyncHandler(async (req, res) => {
  const { status, reason } = req.body;
  sendSuccess(res, maskRateFields(await CrmService.setStatus(req.params.id, status, reason), req, MONEY), `Lead marked ${status.toLowerCase()}`);
});

const convert = asyncHandler(async (req, res) => {
  const result = await CrmService.convert(req.params.id, req.body);
  sendSuccess(res, { ...result, lead: maskRateFields(result.lead, req, MONEY) }, 'Lead converted to a customer', 201);
});
...
const pipeline = asyncHandler(async (req, res) => {
  sendSuccess(res, maskRateFields(await CrmService.pipeline(), req, ['openValuePaise']), 'Pipeline retrieved');
});

...
const ASSET_MONEY = ['costPaise', 'salvageValuePaise', 'accumulatedDepreciationPaise', 'bookValuePaise', 'disposalProceedsPaise', 'gainPaise'];
const RUN_MONEY = ['totalPaise'];
...
const list = asyncHandler(async (req, res) => {
  ...
  sendList(res, req, maskRateFields(data, req, ASSET_MONEY), 'Assets retrieved successfully');
});

const get = asyncHandler(async (req, res) => {
  const asset = await FixedAssetsService.getView(req.params.id);
  await assertCanSeeRecord(req, asset, 'Asset not found');
  sendSuccess(res, maskRateFields(asset, req, ASSET_MONEY), 'Asset retrieved successfully');
});

const create = asyncHandler(async (req, res) => {
  await assertCanUseFactory(req, req.body.factoryId);
  sendSuccess(res, maskRateFields(await FixedAssetsService.create(req.body), req, ASSET_MONEY), 'Asset registered', 201);
});

const update = asyncHandler(async (req, res) => {
  await assertCanSeeRecord(req, await FixedAssetsService.getView(req.params.id), 'Asset not found');
  sendSuccess(res, maskRateFields(await FixedAssetsService.update(req.params.id, req.body), req, ASSET_MONEY), 'Asset updated');
});

const dispose = asyncHandler(async (req, res) => {
  await assertCanSeeRecord(req, await FixedAssetsService.getView(req.params.id), 'Asset not found');
  sendSuccess(res, maskRateFields(await FixedAssetsService.dispose(req.params.id, req.body), req, ASSET_MONEY), 'Asset disposed');
});
...
const preview = asyncHandler(async (req, res) => {
  ...
  sendSuccess(
    res,
    { ...maskRateFields(data, req, RUN_MONEY), lines: maskRateFields(data.lines, req, ['amountPaise', 'bookValueBeforePaise']) },
...
const run = asyncHandler(async (req, res) => {
  await assertCanUseFactory(req, req.body.factoryId);
  sendSuccess(res, maskRateFields(await FixedAssetsService.runDepreciation(req.body), req, RUN_MONEY), 'Depreciation posted', 201);
});
...
const listRuns = asyncHandler(async (req, res) => {
  ...
  sendList(res, req, maskRateFields(data, req, RUN_MONEY), 'Depreciation runs retrieved');
});
...
const cancelRun = asyncHandler(async (req, res) => {
  ...
  sendSuccess(res, maskRateFields(await FixedAssetsService.cancelRun(req.params.id, req.body.reason), req, RUN_MONEY), 'Depreciation run cancelled');
});

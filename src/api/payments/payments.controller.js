...
const listReceipts = asyncHandler(async (req, res) => {
  ...
  const data = await PaymentsService.listReceipts(Number(page), Number(limit), { factoryId, customerPartyId, search });
  sendList(res, req, maskRateFields(data, req), 'Receipts retrieved successfully');
});
const getReceipt = asyncHandler(async (req, res) => {
  sendSuccess(res, maskRateFields(await PaymentsService.getReceipt(req.params.id), req), 'Receipt retrieved successfully');
});
...
const listPayments = asyncHandler(async (req, res) => {
  ...
  const data = await PaymentsService.listPayments(Number(page), Number(limit), { factoryId, partyId, search });
  sendList(res, req, maskRateFields(data, req), 'Payments retrieved successfully');
});
const getPayment = asyncHandler(async (req, res) => {
  sendSuccess(res, maskRateFields(await PaymentsService.getPayment(req.params.id), req), 'Payment retrieved successfully');
});
